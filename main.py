...
class Root(ctk.CTk):
    def __init__(self, **kwargs):
        ...
        self.imgConfiguaracao = ctk.CTkImage(
            Image.open('uix/configuracaop.png'))

        # Configuaracao da janela
        self.title('MidiaPlay')
        self.minsize(600, 300)
        self.configure(bg_color='#200937', fg_color='#200937')
        o = self.bind('<Configure>', self.redimensionar)
        ...
        ctk.CTkButton(self.menuDock, text='', image=self.imgHome, width=28,
                      fg_color='transparent', command=lambda: self.mudaLinha_Tela(posy=0)).pack()
        ctk.CTkButton(self.menuDock, text='', image=self.imgMusica, width=28,
                      fg_color='transparent', command=lambda: self.mudaLinha_Tela(posy=28)).pack()
    ...
    def mudaLinha_Tela(self, posy=0, tela=1):
    ...
    def tela1(self, init=False): 
        if init:
            ...
            telaPlaylist.place(x=0,y=0)
            if self.listaMusicas != None:
                for musica in self.listaMusicas:
                    ctk.CTkButton(telaPlaylist,text=musica[0]).pack()
